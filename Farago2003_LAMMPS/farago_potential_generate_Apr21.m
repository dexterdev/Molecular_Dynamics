...
fileID = fopen('farago2003_tabulated_potential_Apr21.dat','w');
...
%symbolic diff from wolfram alpha to avoid silly mistakes.
...
fprintf(fileID,'\n\n');
V_22=(4*0.75*((1.05 ./ r).^12 - (1.05 ./ r).^6))-(4*0.75*((1.05 / 2.5)^12 - (1.05 / 2.5)^6)) - (4*0.75*(-21.5503 + 8.04057 *2.5^6)/2.5^13).*(r-2.5) - (0.5*(4*0.75*(280.154 - 56.284 *2.5^6)/2.5^14).*(r-2.5).^2);
F_22=(0.75/1.75)*(0.723553 + (150.852./r.^13) - (56.284./r.^7) - (0.25294.*r));
A=[N r V_22 F_22]';
A(:,1)=[1;0.000001;0;0];% zero correction 
...
fprintf(fileID,'\n\n');
V_33=(4*1.875*((1 ./ r).^12 - (1 ./ r).^6) - 4*1.875*((1 / 2.5)^12 - (1 / 2.5)^6) - ( ((45*2.5^6-90)/2.5^13).*(r-2.5) )-0.5*((-45*((7*2.5.^6-26)./2.5.^14))).*(r-2.5).^2);
F_33=-(4*1.875*((6 *(-2 + r.^6))./r.^13)  - ( ((45*2.5^6-90)/2.5^13) )-0.5*((-45*((7*2.5.^6-26)./2.5.^14))).*(r-2.5)*2);
A=[N r V_33 F_33]';
A(:,1)=[1;0.000001;0;0];% zero correction 
...
fprintf(fileID,'\n\n');
V_23=4*375*((0.525 ./ r).^2 - (0.525 ./ r).^1) - 4*375*((0.525 / 2.5)^2 - (0.525 / 2.5)^1) - (4*375*(-0.55125 + 0.525 *2.5)/2.5^3).*(r-2.5) - (0.5*(4*375)*(1.65375 - 1.05 *2.5)/2.5^4).*(r-2.5).^2 ;
F_23=-(4*375*( (-0.55125 + 0.525 .*r)./r.^3)  - (4*375*(-0.55125 + 0.525 *2.5)/2.5^3) - (0.5*(4*375)*(1.65375 - 1.05 *2.5)/2.5^4).*(r-2.5)*2 );
A=[N r V_23 F_23]';
A(:,1)=[1;0.000001;0;0];% zero correction 
